...
public class OrderController extends BaseController {
    ...
    public CommonReturnType createorder(@RequestParam(name= "itemId") Integer itemId, @RequestParam(name= "promoId" ,required = false) Integer promoId, @RequestParam(name = "amount") Integer amount) throws BusinessException {
        ...
        if (isLogin == null || !isLogin.booleanValue()) {
            throw new BusinessException(EmBusinessError.USER_NOTLOGIN);
        }

        UserModel userModel = (UserModel) this.httpServletRequest.getSession().getAttribute("LOGIN_USERINFO");

        OrderModel orderModel =  orderService.creatOrder(userModel.getId(),promoId,itemId,amount);
